...
import { X, Upload, CheckCircle2, AlertCircle, Trash2, Save } from 'lucide-react'
...
export default function BulkImportModal({
  ...
  const [step, setStep] = useState('select_file') // select_file, preview, summary
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const { parseParticipantFile } = await import('../utils/bulkImportParser')
      const results = await parseParticipantFile(selectedFile, { existingStudents, existingTeams, existingCategories })
  ...
  const updateRow = async (index, field, value) => {
    ...
    const { validateParticipants } = await import('../utils/bulkImportParser')
    // Since validateParticipants expects an array of raw rows, we simulate that
    const rawRows = parsedRows.map(r => r.data)
    const validated = validateParticipants(rawRows, existingStudents, existingTeams, existingCategories)
  ...
  const handleImport = async () => {
    ...
    try {
      for (const row of parsedRows) {
        ...
        const { data, error } = await supabase.from('students').insert(row.data).select()
        if (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-4xl bg-card rounded-[28px] border border-subtle shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-subtle flex items-center justify-between bg-black/5">
          <div>
            <h3 className="text-xl font-display font-bold text-mainText">Bulk Import Participants</h3>
            <p className="text-sm text-mutedText">Import student data from CSV, XLSX, or PDF</p>
          </div>
          ...
          {step === 'select_file' && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-mutedText text-sm mb-6 max-w-xs">
                Upload a .csv, .xlsx, or .pdf file containing student details.
              </p>
              <input
                type="file"
                ref={fileInputRef}
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-black/5 text-mutedText text-xs uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="px-4 py-3 font-semibold">Chest No</th>
                      ...
                      .map((row, i) => (
                        <tr key={i} className={`hover:bg-black/5 transition ${!row.isValid ? 'bg-red-500/5' : ''}`}>
                          ...
                          <td className="px-4 py-3">
                            <input
                              className="bg-transparent border-b border-transparent focus:border-accent-purple outline-none w-full"
                              value={row.data.chestNo}
                              onChange={(e) => updateRow(row.index, 'chestNo', e.target.value)}
                            />
                          </td>
                          <td className="px-4 py-3">
                            <input
                          ...
                          <td className="px-4 py-3">
                            <input
                              className="bg-transparent border-b border-transparent focus:border-accent-purple outline-none w-full"
                              value={row.data.team}
                              onChange={(e) => updateRow(row.index, 'team', e.target.value)}
                            />
                          </td>
